...
class Trainer:
    ...
    def prepare_dataset(self) -> None:
        ...
        # 只处理 i2va 类型的模型
        if self.args.model_type != "i2va":
            raise ValueError(f"Invalid model type: {self.args.model_type}. Only 'i2va' is supported.")

        self.dataset = I2VADatasetWithResize(
    ...
    # 初始化训练过程中使用的跟踪器（trackers），并将模型的相关信息传递给跟踪器
    ...
    def encode_action(self, action_data: torch.Tensor) -> torch.Tensor:
        """
        Encodes the action sequence into a format that the model can understand.
        
        Args:
            action_data (torch.Tensor): The raw action tensor, shape (T, D)
